pipeline {
    ...
    stages {
        ...
        stage('Lint') {
            steps {
                sh '.venv/bin/flake8 app.py tests --max-line-length=120'
            }
        }
                ...
                sshagent(credentials: ['uta-app-ssh']) {
                    sh '''
                        scp -o StrictHostKeyChecking=no \
                            ${RELEASE} \
                            deployer@${APP_SERVER}:/tmp/${RELEASE}
    ...
    post {
        success {
            echo "Deployment ${BUILD_NUMBER} completed successfully."
        }
